...
export class UpdateUsersUseCase {
    ...
    async execute(dataUser: IUpdateUsersDTO): Promise<void> {
        ...
        if (user.projects_ids.length > 0) {
            const project_is_added = user.projects_ids.find(project_id => project_id === dataUser.project_id);
            if (project_is_added) throw new Error("User is already added to this project", 400, "USER_ALREADY_ADDED_TO_PROJECT");
            else user.projects_ids.push(dataUser.project_id);
        } else user.projects_ids.push(dataUser.project_id);
